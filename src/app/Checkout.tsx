...
const Checkout: React.FC<CheckoutProps> = ({ cart, setCart }) => {
  ...
  const handleNext = () => {
    if (step === 'shipping') {
      ...
      setCart({ items: [], total: 0, subtotal: 0, shipping: 5.00, tax: 0 });
    ...
    <div className="max-w-2xl mx-auto px-6 md:px-10 py-12">
      ...
      <div className="bg-[var(--cotton)] border border-[var(--ink-10)] rounded-lg p-6 space-y-4">
        ...
        <div className="border-t border-b py-4">
          ...
          <div className="flex justify-between text-sm text-muted-dark mb-2">
            <span>Shipping</span>
            <span>${cart.reduce((sum, item) => sum + (item.shipping || 0), 0).toFixed(2)}</span>
          </div>
          ...
          <div className="flex justify-between text-base font-display font-light text-foreground mt-2">
            <span>Total</span>
            <span className="text-gold">${cart.reduce((sum, item) => sum + (item.price * item.quantity) + (item.shipping || 0), 0).toFixed(2)}</span>
          </div>
        </div>
